...
export default {
  ...
  methods: {
    ...
    getHardWarePromise(bareMetalId) {
      return axios.get("/switch/hardwares/" + bareMetalId);
    },
    /**
    ...
    power(opt, row) {
      this.$confirm(this.$t('confirm') + this.$t('power_' + opt) + '?', this.$t('tips'), {
        ...
        HttpUtil.get("/switch/power/" + row.id + "/" + opt, null, (res) => {
          if (res.success) {
    ...
    powerBatch(opt,) {
      if (opt == 'obm') {
        ...
        this.$confirm(this.$t('confirm') + this.$t('power_' + opt) + '?', this.$t('tips'), {
          ...
          that.loadingList = true;
          HttpUtil.post("/switch/power/" + opt, ids, (res) => {
            if (res.success) {
              this.$message.success(this.$t('success'));
            } else {
    ...
    getHardware() {
      HttpUtil.get("/switch/hardwares/" + this.machine.id, null, (res) => {
    ...
    confirmEdit() {
      ...
      if (this.editType == 'edit') {
        HttpUtil.post("/switch/update", this.editObj, (res) => {
          this.detailDrawer = false;
        ...
        HttpUtil.post("/switch/add", this.editObj, (res) => {
          this.detailDrawer = false;
